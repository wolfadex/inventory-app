...
type ElmLandPorts = {
  ...
  elmLand__onMarkdownChanged: Elm.IncomingPort<{
    filepath: string;
    markdown: string;
  }>;
};

type RouterOption = "file-based" | "markdown" | "inertia";
...
export function init<ports extends {}, flags>(
  ...
  if (!node)
    throw new Error(
      `Elm Land: I could not find an element with id="${nodeId}" when initializing your program.`,
    );
  ...
  let router: RouterOption | undefined =
    dataRouter === "markdown"
      ...
      : dataRouter === "inertia"
        ? "inertia"
        : dataRouter == "file-based"
          ? "file-based"
          : undefined;
  if (router === undefined)
    throw new Error(
      `Elm Land: I expected the "data-router" attribute to be: "markdown", "inertia", or "file-based". I found "${dataRouter}" instead!`,
    );
  ...
  if (Object.keys(app).length === 0) {
    if (import.meta.env.DEV) {
      ...
      throw new Error(
        '"The compiler found a problem, please see the browser overlay for details."',
      );
    } else {
  ...
  app.ports?.elmLand__setShouldPreventNavigation?.subscribe(
    function (newValue) {
   ...
   */
  let ports: Elm.App<ports>["ports"] = Object.keys(app.ports || {}).reduce(
    (obj, key) => {
