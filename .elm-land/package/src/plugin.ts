...
export type Options
  = RouterAndOutputOptions & { extra?: ExtraOptions }
...
type ManualRoutes =
  { [url: UrlPattern]: ElmModuleName } & { '*': ElmModuleName }
...
export default (options_?: Options) : Plugin => {
  ...
  const getRelatedFiles = async (originalUrl : string) : Promise<ContentFiles> => {
    ...
    try {
      ...
      filesNeeded = await compileAndRunElmWorkerForFilesNeeded({ url: toUrlFromPath(originalUrl) }, state.projectRoot)
    } catch (err) {}
    
    let filepaths = filesNeeded.folders.flatMap((folder) => {
      let folderPath = path.join(state.projectRoot, 'content', folder.path)
  ...
  let plugin : Plugin = {
        ...
        switch (options.output) {
          case 'spa':
            this.emitFile({
              type: 'asset',
              fileName: `index.html`,
              source: fromHtmlTemplate(options, null, { head: [
                // TODO: Get this from Main's meta
                { name: 'link', attributes: [
                  { key: 'rel', value: 'icon' },
                  { key: 'href', value: '/logo_200.png' }
                ], content: null },
                { name: 'meta', attributes: [{ key: 'name', value: 'viewport' },{key:"content", value:"width=device-width, initial-scale=1" }], content: null } 
              ], htmlAttributes: [] })
            })
            return
          case 'mpa':
            if (options.router === 'markdown') {
              const markdownFiles = getAllFilesInFolder(contentFolder, true, '.md')
              ...
              markdownFiles.forEach(async (filepath) => {
                ...
                const fileName = relativePath === 'README.md' ? 'index' : relativePath.replace(/\.md$/, '')
                const url = toUrlFromPath(relativePath)
                const contentFiles = await getRelatedFiles(url)
                const dataPage = toDataPage(url, filepath, state.projectRoot, contentFiles)
      
                // Emit HTML file
                this.emitFile({
                  type: 'asset',
                  fileName: `${fileName}.html`,
                  source: fromHtmlTemplate(options, dataPage, { head: [
          ...
          console.error('ERROR:', `TODO: Implement "build" for ${options.router} in ${options.output}`)
          process.exit(1)
      }
    },
    ...
    configureServer: async function (server) {
      ...
      let generatedFiles = toGeneratedFiles(state.projectRoot, options).filter(x => x !== null)

      // Listen for "content/**" changes, and emit them to
      // the web browser for hot-reloading
      server.watcher.on('all', (event, filepath) => {
        let isFile = event === 'add' ||  event === 'change' ||  event === 'unlink'

        // RUN ELM CODEGEN IF FILE/FOLDER IS ADDED/RENAMED/DELETED
        if (event !== 'change') {
          let routes = determineRoutesFrom(state.projectRoot, options)
          for (let file of generatedFiles) {
            if (file.foldersToWatch.some(folder => filepath.startsWith(folder))) {
              let content = file.toContent({ routes })
    
              if (content !== state.fsCache[file.filepath]) {
                console.log('💾', '~/' + path.relative(state.projectRoot, file.filepath))
                state.fsCache[file.filepath] = content
                writeFileSync(file.filepath, content, { encoding: 'utf-8' })
              }
            }
          }
        }
        ...
        if (options.router === 'markdown') {
          if (isFile && filepath.startsWith(contentFolder)) {
            ...
            server.hot.send('elm-land:markdown:changed', { url, data: dataPage })
          }
        }
      })
      ...
      let elmLandMiddleware : Connect.NextHandleFunction = async function (req, res, next: Connect.NextFunction) {
        try {
          ...
          const markdownPath = urlToMarkdownPath(originalUrl, state.projectRoot)

          // Fetch the markdown data for the given URL
          const contentFiles = await getRelatedFiles(originalUrl)

          let dataPage = toDataPage(toUrlFromPath(originalUrl), markdownPath, state.projectRoot, contentFiles)
          ...
          if (originalUrl.endsWith('.md.json')) {
            ...
            res.end(fromHtmlTemplate(options, dataPage, htmlOptions), 'utf8')
          }
      ...
      setTimeout(() => {
        if (server.middlewares.stack.length > 0) {
          server.middlewares.stack.splice(server.middlewares.stack.length - 1, 0, { route: '', handle: htmlFallbackMiddleware(state.projectRoot) })
          server.middlewares.stack.splice(server.middlewares.stack.length - 1, 0, { route: '', handle: elmLandMiddleware })
        } else {
          console.warn('ELM LAND:', 'Expected Vite middleware to be registered!')
        }
      })
...
function fromHtmlTemplate(options: Options, dataPage: DataPage | null, htmlOptions: HtmlOptions): string {
  return `<!DOCTYPE html>
...
function urlToMarkdownPath(url: string, projectRoot: string): string {
  ...
  if (cleanUrl.endsWith('.md.json')) {
    return path.resolve(contentFolder, ...cleanUrl.split('/')).slice(0, -'.json'.length)
  } else {
...
function toUrlFromPath(path: string): string {
  let withExtension = path === 'README.md' || path === '/README.md.json'
    ? '/'
...
function getAllFilesInFolder(dir: string, isRecursive: boolean, extension?: string): string[] {
  try {
...
function toDataPage(url: string, filepath: string, projectRoot: string, content: ContentFiles): DataPage | null {
...
function findElmBinary(pathToElmBinary?: string, projectRoot?: string): string {
  if (pathToElmBinary) {
    ...
    let localNodeModules = path.resolve(projectRoot, 'node_modules', '.bin', 'elm')
...
async function compileElmWorker(elmRootFilepath: string, outputFilename: string, projectRoot: string, pathToElmBinary?: string): Promise<string> {
  // Use child_process to compile the worker
  let tempFilepath = path.resolve(tmpdir(), '.elm-land', 'src', outputFilename)
  ...
  await new Promise((resolve, reject) => {
    let elmMakeProcess = spawn(elmBinary, [
      'make',
      elmRootFilepath,
      '--output',
      tempFilepath,
    ...
    elmMakeProcess.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Elm worker compilation failed with code ${code}`))  
      }
    })
  })
...
async function compileAndRunElmWorkerForFilesNeeded(flags: WorkerFilesNeededFlags, projectRoot: string, pathToElmBinary?: string): Promise<FilesNeeded> {
  // Compile the Elm worker
  let tempFilepath = await compileElmWorker(
    path.join(projectRoot, '.elm-land', 'src', 'ElmLand', 'Worker', 'FilesNeeded.elm'),
  ...
  return new Promise(async (resolve, reject) => {
    ...
    console.warn = warn
    let app = worker.default.Elm.ElmLand.Worker.FilesNeeded.init({ flags: flags })
...
function generateElmLandCode(projectRoot : string, options: Options) {
  ...
  for (let file of generatedFiles) {
    ...
    writeFileSync(file.filepath, file.toContent({ routes }), { encoding: 'utf-8' })
  }
}
...
function setupElmLandPackages(projectRoot : string, options : Options) {
  ...
  let packages = [ 'ElmLand.Effect','ElmLand.Program','ElmLand.Subscription','ElmLand.Http' ]
...
function getRoutesFromPagesFolder(projectRoot: string): ManualRoutes {
  ...
  let pageModulePaths : string[][] =
    elmFilesInPagesFolder
      .map(filepath =>
        path.relative(pagesFolder, filepath.slice(0, -'.elm'.length))
          .split(path.sep)
      )

  let routes : ManualRoutes =
    pageModulePaths.reduce(((routes: ManualRoutes, moduleParts: string[]) => {
...
function determineRoutesFrom(projectRoot: string, options: Options): ManualRoutes {
  let routes
    = (options.router === 'manual') ? options.routes
    : (options.router === 'inertia') ? { '*': 'Pages.ALL_' }
...
const toMaybeGeneratedFiles = (projectRoot : string, options : Options) : (GeneratedFile | null)[] =>
  [
  {
    foldersToWatch: [ path.join(projectRoot, 'src', 'Pages') ],
    filepath: path.join(projectRoot, '.elm-land', 'src', 'Route.elm'),
    toContent: ({ routes }) => toRouteModule({ useHashBasedRouting: options.extra?.useHashBasedRouting || false })
  },
  {
    foldersToWatch: [ path.join(projectRoot, 'src', 'Pages') ],
    filepath: path.join(projectRoot, '.elm-land', 'src', 'Route', 'Path.elm'),
    toContent: ({ routes }) => toRoutePathModule({ routes, useHashBasedRouting: options.extra?.useHashBasedRouting || false })
  },
  {
    foldersToWatch: [ path.join(projectRoot, 'src', 'Pages') ],
    filepath: path.join(projectRoot, '.elm-land', 'src', 'Pages.elm'),
    toContent: ({ routes }) => toPagesModule({ routes, router: options.router })
  },
  (options.router === 'markdown') ? {
    foldersToWatch: [ path.join(projectRoot, 'src', 'Pages') ],
    filepath: path.join(projectRoot, '.elm-land', 'src', 'ElmLand', 'Worker', 'FilesNeeded.elm'),
    toContent: ({ routes }) => toElmWorkerForFilesNeeded({ routes })
  } : null
]

const toGeneratedFiles = (projectRoot : string, options : Options) : GeneratedFile[] =>
...
let toUrlSegmentKind = (url : UrlPattern) => url.split('/').map((part : string) => {
  if (part === '*') {
    return 'catch-all'
...
const bySpecificity = (a : [UrlPattern,ElmModuleName], b : [UrlPattern,ElmModuleName]) => {
  let aUrl = a[0]
  let bUrl = b[0]
...
const toRouteParamType = (url: string) => {
  ...
  return ' { ' + paramNames.map(name => `${name} : ${toParamType(name)}`).join(', ') + ' }'
}
...
const toRouteParamMapping = (url: string) => {
  ...
  return ' { ' + paramNames.map(name => `${name} = ${name}`).join(', ') + ' }'
}
...
const toCasePattern = (url: string) => {
  ...
  let segments = url.split('/').filter(part => part !== '').map((segment) => {
    if (segment === '*') {
      return 'all_'
...
const toRoutePathModule = ({ useHashBasedRouting, routes } : { useHashBasedRouting: boolean, routes: ManualRoutes }) => {
...
const toRouteModule = ({ useHashBasedRouting } : { useHashBasedRouting: boolean }) => {
...
let hasParams = (moduleName : string, routes : ManualRoutes) => {
  let associatedUrlForValue = Object.entries(routes).find(([_, value]) => value === moduleName)?.[0]
  return associatedUrlForValue?.includes(':') || associatedUrlForValue?.includes('*')
}

const toPagesModule = ({ routes, router } : { routes : ManualRoutes, router : RouterOption }) => {
  ...
  let pageModuleNamesWithoutError =
  ...
  let pageModuleNames =
    pageModuleNamesWithoutError
      .concat(router === 'markdown' ? [ 'Pages.ERROR_' ] : [])
  ...
  let routeParams = router === 'inertia'
    ? ''
    : ' params'
  ...
  const importsForMarkdownRouter = router === 'markdown' ? `
...
${pageModuleNamesWithoutError.map(modName => `            Route.Path.${toModuleVariant(modName)}${hasParams(modName, routes) ? ' params' : ''} ->
                handleInitForPage
                    { init = ${modName}.init
                    , shared = shared
                    , url = url
...
${pageModuleNamesWithoutError.map(modName => `        ( Route.Path.${toModuleVariant(modName)}${hasParams(modName, routes) ? ' params' : ''}, ${toModuleVariant(modName)} pageMsg, Model_${toModuleVariant(modName)} pageModel ) ->
            handleUpdateForPage
...
${pageModuleNamesWithoutError.map(modName => `        ( Model_${toModuleVariant(modName)} pageModel, Route.Path.${toModuleVariant(modName)}${hasParams(modName, routes) ? ' params' : ''} ) ->
...
import Html
${pageModuleNames.map(x => 'import ' + x).sort().join('\n')}
...
${pageModuleNamesWithoutError.map(modName => `        Route.Path.${dropPagesPrefix(modName).split('.').join('_')}${hasParams(modName, routes) ? ' _' : ''} ->
            ${modName}.decoder
