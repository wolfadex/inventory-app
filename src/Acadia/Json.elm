...
authenticateEncode : () -> Json.Encode.Value
authenticateEncode () =
    Json.Encode.null


authenticateDecode : Json.Decode.Decoder ()
authenticateDecode =
    Json.Decode.succeed ()


authInfoEncode : Backend.AuthInfo -> Json.Encode.Value
authInfoEncode authInfo =
    Json.Encode.object
        {}
...
userIdCodec =
    Serialize.customType (\enc (Backend.UserID val) -> enc val)
        |> Serialize.variant1 Backend.UserID uuidCodec
...
organizationIdCodec =
    Serialize.customType (\enc (Backend.OrganizationID val) -> enc val)
        |> Serialize.variant1 Backend.OrganizationID uuidCodec
        |> Serialize.finishCustomType
